...
export async function generateMetadata({
  ...
  return { title: (await dic((await params).lang)).contacto.titulo };
}

export default async function Pagina({ params }: { params: Promise<{ lang: Lang }> }) {
  const { lang } = await params;
  const d = await dic(lang);
  return <EnConstruccion titulo={d.contacto.titulo} nota={d.contacto.nota} mientras={d.err.mientras} lang={lang} />;
}
