...
| Stack | Next.js 16.2.7 + React 19.2.4 + Supabase + Tailwind + TypeScript + Vercel |
| Estado | pausado |
| Último commit | 2026-09-07 |
...
- `wordpress-staging-y-wp-cli-agentico` — **WordPress ajeno — relevar, probar y trabajar agéntico sin romper la tienda**
  Cuando un cliente entrega las credenciales de **un WordPress que ya existe** y hay que intervenirlo…
...
- `title-rule-adaptive-width` — **Filete adaptativo con desvanecimiento asimétrico**
  Cuando necesitas una línea decorativa (filete, rule, underline) debajo de títulos o secciones que: Se adapte…
...
- `identidad-sin-cuentas-token-y-mail` — **Identidad sin cuentas — token, mail y qué revela cada puerta**
  Cuando una app tiene que reconocer personas **sin login**: links personales del tipo `/c/<token>`, QR pegados…
...
- `carousel-framer-motion-aspect-ratio` — **Carrusel full-bleed con relación de aspecto fija (Framer Motion)**
  Cuando necesitas un carrusel que: Pase diapositivas automáticamente con transiciones fluidas Respete una…
